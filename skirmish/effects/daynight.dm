			...
			if(active_game.started == 2)
				if(active_game.boss_mode)
					if(alpha != max_darkness)
						...
						if(p) p.play_sound('audio/sounds/thunder.ogg')
						animate(src, color = "#000000", alpha = 100, time = 2, easing = ELASTIC_EASING)
					...
					animate(src, color = ambient_color, alpha = n_alpha, time = 5)
