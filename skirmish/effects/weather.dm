	...
	weather_loop()
		...
		if(active_game.weather_turfs.len && active_game.started == 2)
			while(active_game.started == 2)
				if(active_game.toggle_weather && active_game.weather_turfs.len)
					var/turf/t = pick(active_game.weather_turfs)
					active_game.weather_turfs -= t
...
turf
	proc
		...
		snow()
			...
			animate(transform = transform/4, pixel_y = 200, pixel_x = -8, alpha = 210, time = 30.4)
