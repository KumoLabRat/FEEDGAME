		...
		if(turf_list.len) for(var/i = 0 to flames)
			...
			switch(fire_count(t))
				if(0)
					...
					f.spawndel(ds)
					for(var/obj/hazard/oil/o in obounds(t,16))
						if(o.lit) continue // don't light trails that are already lit.
						o.lit = 1
						spawn(5)
							o.drop_fire(1, o.owner)
							o.spawndel(5)
				if(1)
...
mob
	...
	proc
		...
		freeze()
			set waitfor = 0
			if(!frozen && can_freeze)
				frozen			= 1
				move_disabled	= 1
				. = icon
				icon = New(icon,icon_state,dir,2,0)
			//	overlays += SLOWED_OVERLAY
				var/i = 0
				while(health && frozen)
					if(i > 4) break
